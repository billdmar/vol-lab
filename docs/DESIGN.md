...
**Measured cross-engine differentials:** CRR→BS convergence order **0.9993**
...
The external verifier this project is built around: our independently-written IV solver's
...
Verification → tolerance map: **engine cross-verification** exercises `parity_model` (~2e-14 ≪ 1e-10),
...
`greeks_mc_vs_closed_rel` (<0.5%), `iv_roundtrip_abs`. **surface verification** exercises
