...
It is a **derivatives / quant-research** project centered on options-pricing fundamentals —
put-call parity, Greeks intuition, implied vol, and the smile. The organizing principle is
...
- **Tolerance registry** — `config/tolerances.py` holds **13** named tolerances, each a
  ...
  investigated and written up. The verification→tolerance map is in [`DESIGN.md`](DESIGN.md).
...
- **Engine cross-verification** (`tests/test_integration_engines.py`) — binomial→BS measured
...
- **Surface verification** (`tests/test_integration_surface.py`) — the full pipeline on the
...
Suggested reading order: **README → this doc → DESIGN → RESEARCH_NOTE**.
