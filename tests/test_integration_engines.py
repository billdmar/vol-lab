"""Engine cross-verification (integration tests).
...
the exchange. The four cross-engine checks:
...
def test_crr_converges_to_bs_measured_order():
    ...
    order = fit["order"]
    print(f"\n[engines] CRR->BS measured convergence order = {order:.4f} "
...
def test_mc_ci_covers_closed_form_across_grid():
    ...
    frac = covered / total
    print(f"\n[engines] MC 95% CI coverage of BS price = {covered}/{total} = {frac:.2f} "
...
def test_greeks_closed_form_vs_mc_pathwise():
    ...
    lr_gamma = mc.lr_gamma(spot=SPOT, strike=strike, tau=tau, rate=0.0,
                           sigma=sigma, option_type="C")
    print("\n[engines] Greeks 3-way (ATM tau=0.75 sig=0.65):")
...
def test_american_ge_european_cross_engine():
    ...
    print(f"\n[engines] American put {pa:.4f} >= European put {pe:.4f} (BS {p_bs:.4f}); "
