"""Surface verification (integration tests).

Ties the surface, no-arb scanner, and exchange differential together on the committed
Deribit fixtures and asserts its acceptance criteria:
...
These are descriptive checks (exch_diff has no pass/fail tolerance per the registry); the
...
def test_determinism_same_fixture_identical_stats():
